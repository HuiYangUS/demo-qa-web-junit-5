...
public class ApplyTest extends DriverFactoryWebBase {
	...
	@Test
	@Tag("email")
	void applyNegativeEmailTest() {
		driver.navigate().to(boratechURL);
		driver.findElement(By.xpath("//a[@href='/apply']")).click();
		wait.until(ExpectedConditions.urlContains("/apply"));
		String validationText = driver.findElement(By.name("email")).getAttribute("validationMessage");
		assertTrue(validationText.isBlank(), "Invalid displayed validatation message.");
		// data entries
		driver.findElement(By.name("firstname")).sendKeys(firstName);
		driver.findElement(By.name("lastname")).sendKeys(lastName);
		driver.findElement(By.name("dob")).click();
		...
		// submit
		driver.findElement(submitByLocator).click();
		webUtils.mouse().scrollToElement(driver.findElement(By.name("email"))).perform();
		wait.until(ExpectedConditions.attributeToBeNotEmpty(driver.findElement(By.name("email")), "validationMessage"));
		validationText = driver.findElement(By.name("email")).getAttribute("validationMessage");
		assertFalse(validationText.isBlank(), "Failed to display validatation message.");
